...
export default function PastTrips() {
  ...
  const items = useSelector(selectBookings);
  ...
  useEffect(() => {
    ...
    (async () => {
        ...
        rows.map(async (b) => {
          ...
          try {
            ...
            prop = {
              ...
              price_per_night: p.price_per_night != null ? Number(p.price_per_night) : 0,
      ...
      if (!cancelled) setViewItems(enriched);
    })();
  ...
  async function cancelBooking(id) {
    ...
    setViewItems((list) => list.map((b) => (b.id === id ? { ...b, status: "Cancelled" } : b)));
    try {
      await fetchJSON(
  ...
  const hasTrips = useMemo(() => viewItems.length > 0, [viewItems]);
              ...
              <div className="row g-3 align-items-center">
                ...
                <div className="col-auto text-end">
                  ...
                  <div className="mt-2 d-flex gap-2 justify-content-end">
                    <Link className="btn btn-outline-secondary btn-sm rounded-pill" to={`/property/${p.id}`}>
                      View place
                    </Link>
